...
export function home(state){
  ...
  <section class="home-section-v5 lab-atlas"><header class="section-head"><div><span class="eyebrow">${bi(`${Object.keys(categories).length} ЛАБОРАТОРИЙ`,`${Object.keys(categories).length} LABORATORIES`,lang)}</span><h2>${bi('Одна система, разные решения','One system, different decisions',lang)}</h2></div><a class="text-action" href="#calculators">${bi(`Все ${CALCULATORS.length} инструментов`,`All ${CALCULATORS.length} tools`,lang)} ${icon('arrow')}</a></header><div class="lab-atlas-grid">${Object.entries(categories).map(([id,category])=>`<a href="#category/${id}" class="domain-${id}"><img src="${DOMAIN_CONTENT[id].asset}" width="1200" height="900" loading="lazy" alt=""><span>${icon(id)}</span><div><strong>${esc(l(category,lang))}</strong><small>${esc(l(category.question,lang))}</small></div><b>${count(id)}</b></a>`).join('')}</div></section>
